...
class FighterCollectionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        fighterCollectionView.register(FighterCollectionViewCell.nib(),
                                       forCellWithReuseIdentifier: FighterCollectionViewCell.reuseIdentifier)
...
extension FighterCollectionViewController: UICollectionViewDelegate,
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        if let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: FighterCollectionViewCell.reuseIdentifier,
                for: indexPath) as? FighterCollectionViewCell {
            ...
            if selectedFighter.picturesList.count>0 {
                selectedFighterImage = Utility.retriveImage(
                                    imageName: selectedFighter.picturesList[0]) ?? UIImage(named: "no_image_available")!
            } else {
            ...
            print("FighterCollectionViewController Cell is nil")
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        let width = collectionView.frame.width -
                    ...
                    collectionView.safeAreaInsets.left - 30
        return CGSize(width: width, height: 300)
    }
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let selectedFighter = self.flightList[indexPath.row]
        if let viewController = storyboard?.instantiateViewController(
                            identifier: FighterDetailViewController.reuseIdentifier)
                            as? FighterDetailViewController {
            viewController.selectedFighter = selectedFighter
            navigationController?.pushViewController(viewController, animated: true)
        } else {
            print("Fighter Detail View Controller Can't be initiated")
        }
    }
}
